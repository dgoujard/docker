...
All following examples will bring you phpMyAdmin on `http://localhost:80`
...
```
docker run --name myadmin -d --link mysql_db_server:db -p 80:80 damien/phpmyadmin
```
...
```
docker run --name myadmin -d -e PMA_HOST=dbhost -p 80:80 damien/phpmyadmin
```
...
```
docker run --name myadmin -d --link mysql_db_server:db -p 8080:8080 -e PMA_ARBITRARY=1 damien/phpmyadmin
```
